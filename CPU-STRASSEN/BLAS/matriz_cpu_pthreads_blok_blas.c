...
void matmul_blas(double* A, double* B, double* C, int size) {
    ...
    // CblasRowMajor: matrizes armazenadas por linhas
    // CblasNoTrans: não transpor as matrizes
    // size: dimensão das matrizes (size x size)
    // 1.0: alpha, 0.0: beta
...
void* thread_multiply(void* arg) {
    ...
    // Cada thread processa um bloco de linhas da matriz C
...
int main(int argc, char *argv[]) {
    ...
    strassen(A, B, C, N);
    double end = get_time();
    printf("Strassen com BLAS e Pthreads (Threads: %d, Block Size: %d): Tempo = %.4f s\n", 
