...
int N;
int block_size; // ✅ novo parâmetro global
...
int main(int argc, char *argv[]) {
    ...
    N = atoi(argv[1]);
    block_size = atoi(argv[2]); // ✅ lê o block_size
    ...
    strassen(A, B, C, N);
    double end = get_time();
    printf("Strassen Pthreads + Blocking (block_size = %d): %.4f s\n", block_size, end - start);
