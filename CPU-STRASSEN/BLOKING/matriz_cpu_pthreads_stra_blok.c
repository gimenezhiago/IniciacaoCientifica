...
// Multiplicação convencional COM BLOCKING
void conventional_mult_blocking(double **A, double **B, double **C, int size) {
...
// Função executada pelas threads (usando blocking)
void* threaded_mult_blocking(void* arg) {
    MatrixArgs *args = (MatrixArgs *)arg;
    conventional_mult_blocking(args->A, args->B, args->C, args->size);
    pthread_exit(NULL);
}
...
void strassen(double **A, double **B, double **C, int size) {
    if (size <= block_size) {
        ...
        // Inicializa C com zeros (necessário para blocking)
        for (int i = 0; i < size; i++)
            for (int j = 0; j < size; j++)
                C[i][j] = 0.0;
        ...
        for (int t = 0; t < num_threads; t++) {
            ...
            args[t].C = C;
            args[t].size = size;
            pthread_create(&threads[t], NULL, threaded_mult_blocking, &args[t]);
        }
...
int main(int argc, char *argv[]) {
    ...
    strassen(A, B, C, N);
    double end = get_time();
    printf("Strassen com Pthreads + Blocking (block_size=%d, %d threads): Tempo = %.4f s\n", block_size, num_threads, end - start);
